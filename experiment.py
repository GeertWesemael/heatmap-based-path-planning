...
for start_times in [hour_min_to_sec(10, 0),hour_min_to_sec(10, 4),hour_min_to_sec(10, 12)]:

    # for intervals in [10,30,
    ...
    #                 hour_min_to_sec(0, 40),hour_min_to_sec(0, 50),hour_min_to_sec(1, 0)]:
    ...
    for value in range(1, 30):
        ...
        if amount_of_robots == 1:
            ...
            if experiment == "wait":
                coll = 0
                time = 0
                for w in world_tests:
                    r1 = robot.Robot(start_loc_robot, robot_start_time, map1)
                    r1.plan_path_waiting_at_encounter(w, end_loc_robot, sample_r_wait, distance_robot, wait_time)
                    coll += r1.evaluate_collisions(w, sample_r_col)
                    time += r1.evaluate_time()
                    # w.plot_world(fr=hour_min_to_sec(10,0),to=hour_min_to_sec(10,1))
                coll = coll/len(world_tests)
                time = time/len(world_tests)
